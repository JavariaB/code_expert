...
try {
	if (isset($_GET['action']) && $_GET['action'] == 'delete' && isset($_GET['id']) && !empty($_GET['id'])) {
		$sql = $conn->prepare('DELETE FROM diet WHERE `user_id` = ? AND `id` = ?');
	...
	$sql = $conn->prepare('SELECT * FROM diet WHERE `user_id` = ? LIMIT 10 OFFSET ?');
	...
	$totalRecordsSql = $conn->prepare('SELECT COUNT(*) AS count FROM diet WHERE `user_id` = ?');
...
<head>
	...
	<title>Fresh Organic - Diet List</title>
						...
						<div class="col-lg-12 col-md-12 mb-5">
							<div class="blog-item">
								<div class="blog-item-content">
									<h2 class="mb-3">Diet Plan</h2>

									<p class="mb-4">This is your current list of diet plans.</p>
									...
									<table class="table table-bordered" Style="background-color: #f8f9fa">
										<thead>
											<tr style="background-color: #17a050">
												...
												<th scope="col">Previous Iteme</th>
										...
										<tbody>
											<?php if ($result->num_rows > 0) : ?>
												<?php while ($diet = $result->fetch_assoc()) : ?>
													<tr>
														<th scope="row"><?= ++$key + (($page - 1) * 10) ?></th>
														<td><?= ucwords($diet['name']) ?></td>
														<td><?= $diet['previous_item'] ?></td>
														<td><?= $diet['new_item'] ?></td>
														<td><?= date('d-M-Y', strtotime($diet['created_at'])) ?></td>
														<td>
															<a href="admin/diet/list.php?id=<?= $diet['id'] ?>&action=delete" class="text-danger" onclick="return confirm('Are you sure you want to delete this item?');">Delete</a>
														</td>
													</tr>
												<?php endwhile; ?>
											<?php else : ?>
									...
									<nav aria-label="Page navigation example">
										<ul class="pagination">
											<li class="page-item">
												<a class="page-link" Style="background-color: #E5E0F5" href="admin/diet/list.php?page=<?= isset($_GET['page']) && (int)$_GET['page'] > 1 ? (int)$_GET['page'] - 1 : 1 ?>">Previous</a>
											</li>
											<?php for ($i = 1; $i <= ceil($count['count'] / 10); $i++) : ?>
												<li class="page-item">
													<a class="page-link" Style="background-color: #E5E0F5" href="admin/diet/list.php?page=<?= $i ?>"><?= $i ?></a>
												</li>
											<?php endfor; ?>
											<li class="page-item">
												<a class="page-link" Style="background-color: #E5E0F5" href="admin/diet/list.php?page=<?= isset($_GET['page']) && (int)$_GET['page'] < ceil($count['count'] / 10) ? (int)$_GET['page'] + 1 : ceil($count['count'] / 10) ?>">Next</a>
											</li>
										</ul>
									</nav>
